...
export const ImagePreview: React.FC<ImagePreviewProps> = ({
  ...
  return (
    <div
      className={`w-full h-full flex items-center justify-center bg-gray-50 rounded-lg border ${className}`}
    >
      <img
        src={url}
...
export const FilePreview: React.FC<FilePreviewProps> = ({
  ...
  if (signedUrl && file.mime.startsWith("image/")) {
    return (
      <ImagePreview url={signedUrl} alt={file.name} className={className} />
    );
  }

  if (signedUrl && file.mime === "application/pdf") {
    return (
      <PDFPreview url={signedUrl} title={file.name} className={className} />
    );
  }
  ...
  return (
    <div
      className={`w-full h-full flex flex-col items-center justify-center bg-gray-50 rounded-lg border p-8 ${className}`}
    >
