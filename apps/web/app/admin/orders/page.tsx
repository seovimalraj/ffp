...
const Page = () => {
  ...
  }, [filters.status, filters.search, fetchOrders]);

  // Also fetch statuses when search or status filter changes to keep cards updated?
  // Actually, status cards should probably just show totals for the whole system or be affected by search?
  // Portal logic re-fetches statuses on filter change. Let's stick to that.
  useEffect(() => {
    fetchStatuses();
  }, [filters.search]);
  ...
  const columns: Column<IOrder>[] = [
    ...
    },
    {
      ...
      header: "Organization",
      render: (row) => (
        <span className="font-semibold">{row.organization_name}</span>
      ),
    },
    {
      key: "total_amount",
        ...
        <div className="flex flex-1 flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-1 p-1 bg-gray-100/50 dark:bg-gray-800/40 rounded-xl relative border border-gray-200/50 dark:border-gray-700/50">
            ...
            <Input
              ...
              placeholder="Search by code or org..."
              className="h-8 border-none bg-transparent focus-visible:ring-0 text-sm font-medium w-64 shadow-none"
            />
          </div>
