...
/**
 * Proxy to the CAD service's deterministic sheet-metal analysis.
 *
 *   POST /api/cad/analyze-sheet-metal   -> POST {CAD_SERVICE_URL}/api/v1/cad/analyze-sheet-metal
 *   GET  /api/cad/analyze-sheet-metal   -> GET  {...}/analyze-sheet-metal/capabilities
 *
...
const ANALYZE_PATH = "/api/v1/cad/analyze-sheet-metal";
const CAPABILITIES_PATH = "/api/v1/cad/analyze-sheet-metal/capabilities";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!contentType.toLowerCase().includes("multipart/form-data")) {
    return errorResponse(
      ...
      "Send multipart/form-data with a `file` field. Optional fields: " +
        ...
        "include_debug_geometry, include_topology_entities.",
  ...
  try {
    ...
    console.error("[analyze-sheet-metal] upstream request failed", error);
  ...
  try {
    ...
    console.error(
      "[analyze-sheet-metal] non-JSON response from CAD service",
      upstream.status,
      raw.slice(0, 500),
    );
    return errorResponse(
...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[analyze-sheet-metal] capabilities request failed", error);
    return upstreamFailure(error, target);
  }
}
