...
import type {
  ...
  SheetMetalCapabilities,
  SheetMetalAnalysisResult,
} from "@/types/sheet-metal-analysis";
import { isSheetMetalError } from "@/types/sheet-metal-analysis";

import { AnalysisPanel } from "./components/analysis-panel";
import { formatBytes, formatDuration } from "./lib/format";

/**
 * CAD sheet-metal analysis workbench.
 *
 * Uploads a B-Rep file to `/api/cad/analyze-sheet-metal` (which proxies the
 * CAD service), renders the solid in the shared CadViewer, and shows the
 * extracted geometry alongside it.
 *
...
type Status =
  | { kind: "idle" }
  | { kind: "analyzing" }
  | { kind: "done"; result: SheetMetalAnalysisResponse }
  | { kind: "error"; code: string; message: string };

export default function SheetMetalAnalysisPage() {
  const [file, setFile] = useState<File | null>(null);
  const [status, setStatus] = useState<Status>({ kind: "idle" });
  const [capabilities, setCapabilities] = useState<SheetMetalCapabilities | null>(
    null,
  );
  ...
  useEffect(() => {
    let cancelled = false;
    fetch("/api/cad/analyze-sheet-metal")
      .then((response) => (response.ok ? response.json() : null))
      .then((data) => {
        if (!cancelled && data && typeof data.kernel_available === "boolean") {
          setCapabilities(data as SheetMetalCapabilities);
        }
      })
      .catch(() => {
  ...
  const analyze = useCallback(
    async (target: File, withFaceDetails: boolean) => {
      ...
      try {
        ...
        const payload = (await response.json()) as SheetMetalAnalysisResult;

        if (!response.ok || isSheetMetalError(payload)) {
          const error = isSheetMetalError(payload) ? payload.errors[0] : undefined;
          setStatus({
            kind: "error",
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Deterministic sheet-metal geometry extraction — no cost or
              pricing
            </p>
          </div>
        </div>
            ...
            <section className="relative min-w-0 flex-1 bg-white">
              ...
              {status.kind === "done" && (
                <div className="absolute bottom-4 left-4 rounded-lg border border-slate-200 bg-white/95 px-3 py-2 text-[11px] text-slate-600 shadow-lg backdrop-blur">
                  ...
                  {" · "}
                  {status.result.sheet_metal_candidate.is_candidate
                    ? "sheet-metal candidate"
                    : "not a sheet-metal candidate"}
                </div>
              )}
            </section>
              ...
              {status.kind === "analyzing" && (
                <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center">
                  ...
                  <p className="max-w-[260px] text-xs leading-relaxed text-slate-500">
                    Reading the B-Rep, scoring sheet-metal candidacy and
                    recognising bends and flat-pattern features. Large
            ...
            >
              ...
              <div>
                ...
                <p className="mt-1.5 text-sm leading-relaxed text-slate-500">
                  Extracts bends, flanges, holes, cutouts, slots and hems, plus
                  DFM distance flags — deterministically, from the B-Rep.
                </p>
              </div>
              <button
                onClick={open}
