...
export type EditWorkflowTemplateModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
...
export const EditWorkflowTemplateModal = ({
  isOpen,
  onClose,
  onSuccess,
  templateId,
}: EditWorkflowTemplateModalProps) => {
  ...
  const fetchTemplateDetails = async () => {
    setFetching(true);
    try {
      const response = await apiClient.get(`/order-workflows/templates/${templateId}`);
      if (response.data.success) {
        const { name, description, is_active, phases: savedPhases } = response.data.data;
        setFormData({ name, description, is_active });
        // Map saved phases to include local 'id' for DnD if not present, though we should use consistent keys
        setPhases(savedPhases.map((p: any) => ({ ...p, id: p.id || uuidv4() })));
        setStep(0);
        setEditingId(null);
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiClient.patch(`/order-workflows/templates/${templateId}`, payload);
      notify.success("Workflow template updated");
      onSuccess?.();
      onClose();
    } catch (error: any) {
      notify.error(
            ...
            <div className="px-10 py-8 border-t border-slate-100 dark:border-neutral-800 bg-slate-50/50 dark:bg-neutral-900/50 flex justify-between items-center">
              ...
              <div className="flex gap-3">
                ...
                {step === 0 ? (
                  ...
                  >
                    Review Stages <ChevronRight className="w-4 h-4" />
                  </Button>
                ) : (
                  <Button
