...
const leadTimeMultiplierMap = { economy: 0.9, standard: 1, expedited: 1.25 } as const;
const shippingDaysByType = { economy: 7, standard: 5, expedited: 2 } as const;
...
export function calculatePricing(input: PricingInput): PricingBreakdown {
  ...
  // 7. Overhead Cost (15% of direct costs - optimized operations)
  ...
  const overheadCostPerUnit = directCosts * 0.15;

  // 8. Margin (12% of total - competitive but sustainable)
  const costBeforeMargin = directCosts + overheadCostPerUnit;
  const marginCostPerUnit = costBeforeMargin * 0.12;
  ...
  const leadTimeMultiplier = leadPlan.multiplier;
...
function weightDiscount(rawWeightKg: number, quantity: number, subtotalPerUnit: number): { quantityDiscountRate: number; quantityDiscount: number } {
  ...
  // Enhanced discount tiers for competitive pricing
  if (totalWeightKg >= 200) rate = 0.25;
  else if (totalWeightKg >= 100) rate = 0.20;
  else if (totalWeightKg >= 50) rate = 0.16;
  else if (totalWeightKg >= 20) rate = 0.12;
  else if (totalWeightKg >= 10) rate = 0.08;
  else if (totalWeightKg >= 5) rate = 0.05;
  else if (quantity >= 10) rate = 0.03;

  return { quantityDiscountRate: rate, quantityDiscount: subtotalPerUnit * rate };
}
...
function computeLeadTime(
  ...
  const rawLead = productionDays + shippingDays + bufferDays;
  const leadTimeDays = Math.max(7, rawLead);

  return {
    leadTimeDays,
    multiplier: leadTimeMultiplierMap[leadTimeType],
    components: { productionDays, shippingDays, bufferDays }
  };
}
