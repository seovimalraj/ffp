...
const MATERIAL_DENSITIES: Record<string, number> = {
  ...
  "stainless-304": 0.000008, // 8.0 g/cc
  "stainless-316": 0.000008, // 8.0 g/cc
...
function estimateMachineTimeMinutes(metrics: GeometryMetricsV1): number {
  ...
  if (metrics.volume_cc && metrics.volume_cc > 0) {
    ...
    const removalTime =
      (metrics.volume_cc / DEFAULT_REMOVAL_RATE_CC_PER_MIN) * 0.4;
    time += removalTime;
...
function estimateMaterialMassKg(
  metrics: GeometryMetricsV1,
  materialKey?: string,
): number {
...
function selectQuantityDiscount(
  factors: CostFactorsV1,
  quantity: number,
): number {
  if (!factors.quantity_breaks || factors.quantity_breaks.length === 0)
    return 0;
  let applicable = 0;
...
export function computePricingBreakdown(
  ...
  const machine_time_min =
    overrides?.machine_time_min ?? estimateMachineTimeMinutes(metrics);
  ...
  const material_mass_kg =
    overrides?.material_mass_kg ?? estimateMaterialMassKg(metrics, materialKey);

  // Component costs (per part)
  const tolMul =
    tolerance_multiplier && tolerance_multiplier > 0 ? tolerance_multiplier : 1;
  const machining =
    (machine_time_min / 60) * factors.machine_rate_per_hour * tolMul;
  ...
  const unit_cost_before_margin =
    material + machining + setup + finish + inspection + overhead;
